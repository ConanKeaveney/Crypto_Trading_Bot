...
import getopt
import datetime
from cryptotrading.src.Crypto_Trading_Bot_py3.plot_chart.poloniex import poloniex
...
class Chart(object):
    def Generate(self):
        ...
        try:
            opts, args = getopt.getopt(sys.argv[1:], "hp:c:n:s:e:", [
        ...
        output = open(settings.WORKPATH + "/projectmanager/cryptotrading/templates/trading/chart.html", 'w')
        output.truncate()
        output.write(
        ...
        while True:
            if (startTime and historicalData):
                ...
                for point in dataPoints:
                    output.write("['"+point['date']+"',"+point['price'] +
                                ","+point['label']+","+point['desc']+","+point['trend'])
                    output.write("],\n")
            ...
            dataPoints.append({'date': dataDate, 'price': str(lastPairPrice), 'trend': str(
                currentResistance), 'label': 'null', 'desc': 'null'})
